...
static int at_exec_join(char *str)
{
	...
	API_LOG("API", "Join request = %d", bJoin);
	if (bJoin != 1 && bJoin != 0)
	{
		return AT_ERRNO_PARA_VAL;
	}
	...
	if (param != NULL)
	{
		...
		API_LOG("API", "Autojoin = %d", autoJoin);
		...
		if (param != NULL)
		{
			...
			uint8_t join_int = strtol(param, NULL, 0);
			API_LOG("API", "Join interval = %d", join_int);
			...
			if (param != NULL)
			{
				...
				API_LOG("API", "Join attempts = %d", nbtrials);
