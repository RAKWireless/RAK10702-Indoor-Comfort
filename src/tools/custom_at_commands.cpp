/**
 * @file custom_at_commands.cpp
 * @author Bernd Giesecke (bernd@giesecke.tk)
 * @brief Setup and handle user defined AT commands
 * @version 0.2
 * @date 2024-02-21
 *
			...
			if (param != NULL)
			{
				...
				if (param != NULL)
				{
					...
					if (param != NULL)
					{
						...
						MYLOG("USR_AT", "Set RTC to %d.%02d.%02d %02d:%02d", year, month, date, hour, minute);
...
static int at_set_co2(char *str)
{
	...
	bool success = force_calib_rak12037(new_cal);
...
int at_query_co2(void)
{
	...
	snprintf(g_at_query_buf, ATQUERY_SIZE, "%d", current_calib_value);
